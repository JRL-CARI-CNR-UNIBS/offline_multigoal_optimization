  ...
  for (std::string& tf_name: tf_list)
  {
    ...
    for (int isol=0;isol<number_ik;isol++)
    {
      ...
      for (const std::pair<double,std::tuple<std::string,Eigen::VectorXd,double*,int>>& p: ordered_goal)
      {
        ...
        ROS_INFO("Now considering from %s/%u to %s/%u",tf_name.c_str(),isol,tf_name2.c_str(),isol_dest);
...
//        }
        ...
        pathplan::SamplerPtr sampler=std::make_shared<pathplan::InformedSampler>(tree->getRoot()->getConfiguration(),
                                                                                 ...
                                                                                 lb,
                                                                                 ub);
        if (sampler->getFociiDistance()>(*best_goal_from_tree_to_tree2)*best_cost_from_this_goal)
        {
        ...
        {
          ROS_INFO("   skipped because is too far: from %s/%u to %s/%u",tf_name.c_str(),isol,tf_name2.c_str(),isol_dest);
